...
namespace rdf {
    ...
    struct trie<Mapper, Levels>::iterator {
        iterator(uint64_t first,
                 typename Levels::second::iterator const& second,
    ...
    template<typename Mapper,
             typename Levels>
    typename trie<Mapper, Levels>::iterator
             trie<Mapper, Levels>::select_all()
    {
    ...
    template<typename Mapper,
             typename Levels>
    typename trie<Mapper, Levels>::iterator
             trie<Mapper, Levels>::select(triplet const& t)
    {
        ...
        {
            num_triplets = second.pointers[r.end - 1].end -
                           second.pointers[r.begin].begin;
        }
    ...
    struct trie<Mapper, Levels>::iterator_so {
        iterator_so(uint64_t first,
                    ...
                    typename Levels::third::nodes_type& nodes,
                    uint64_t third,
    ...
    template<typename Mapper,
             typename Levels>
    typename trie<Mapper, Levels>::iterator_so
             trie<Mapper, Levels>::select_so(triplet const& t)
    {
    ...
    struct trie<Mapper, Levels>::iterator_po {
            ...
            , m_trie(data)
        {
            ...
            m_second_it = typename Levels::second::iterator(
                (m_trie->second).nodes.begin(),
                (m_trie->first).pointers.begin()
            );
        }
    ...
    template<typename Mapper,
             typename Levels>
    typename trie<Mapper, Levels>::iterator_po
             trie<Mapper, Levels>::select_p(triplet const& t)
    {
        assert(id() == permutation_type::spo);
        assert(t.first  == global::wildcard_symbol);
        assert(t.second != global::wildcard_symbol);
    ...
    template<typename Mapper,
             typename Levels>
    typename trie<Mapper, Levels>::iterator_po
             trie<Mapper, Levels>::select_o(triplet const& t)
    {
